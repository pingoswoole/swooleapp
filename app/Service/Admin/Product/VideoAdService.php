...
class VideoAdService extends Base
{
    
    protected $model_class = \App\Model\Product\VideoAd::class;
    /**
     * 获取所有
     *
     * @author pingo
     ...
     */
      public function getPageList($page, $pageSize, array $where = []):array
      {
          ...
          try {
              ...
              $list = $this->model->where($where)->orderBy('id', 'DESC')->page($page, $pageSize)->with(['area'])->get();
      ...
      public function addItem(array $data):bool
      {
            try {
                //code...
                if(is_string($data['resource_urls'])) $data['resource_urls'] = [$data['resource_urls']];
