...
class MessagePushService extends Base
{
    
    protected $model_class = \App\Model\Product\MessagePush::class; 
     ...
     */
      ...
      public function addItem(array $data):bool
      {
            try {
                ...
                if($id > 0){
                    //添加推送
                    (new UniPushLogic)->pushMessageToApp($data['title'], $data['content']);
                }
            } catch (\Throwable $th) {
                //throw $th;
                var_dump($th->getMessage(), $th->getLine(), $th->getFile());
                return false;
            }
            return true;
      }
