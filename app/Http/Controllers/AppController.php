...
class AppController extends Controller
{
    ...
    public function checkAuth()
    {
        ...
        if (isset($headers['token'])) {
            ...
            if (false !== $token_res) {
                $this->auth_user_data = (new Member())->where('id', $token_res['id'])->first();
                
                if ($this->auth_user_data['state'] == - 1) {
                    $this->json(Status::CODE_SESSION_EXP, '账号已禁用');
                    return false;
                }
     ...
     * @return void
     */
    protected function json($code = 0, $msg = 'success', $data = null, $err_code = 0, $err_msg = null)
    {
        $format = [
            'code'      => $code,
            'msg'       => $msg,
    ...
    {
        $this->json($data['status'], $data['msg'], $data['data'], $data['err_code'], $data['err_msg']);
    }
    /**
     * 分页数据返回
     *
