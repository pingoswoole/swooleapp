...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#dataTable',
            url: '/backadmin/member/member_address/getPageList',
            ...
            cols: [[
                ...
                {field: 'is_default',  title: '是否默认', templet: function(data){
        ...
        table.on('toolbar(dataTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backadmin/member/member_address/addItem',
                    end: function(){
                        table.reload('dataTable')
                    }
                });
        ...
        form.on('switch(stateSwitch)', function(obj){
            ...
            $.post('/backadmin/member/member_address/setItem/' + obj.value ,datajson,function(data){
                layer.msg(data.msg);
                if(data.code !== 0) {
        ...
        table.on('tool(dataTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backadmin/member/member_address/viewItem/'+data.id,
                    end: function(){
                        table.reload('dataTable')
                    }
                });
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.post('/backadmin/member/member_address/delItem/'+data.id, {}, function(result){
