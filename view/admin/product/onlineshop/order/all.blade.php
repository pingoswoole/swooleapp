...
<script type="text/html" id="dataTableBar">
    ...
    @verbatim
    {{#  if(d.state === 1){ }}
    <a class="layui-btn layui-btn-normal layui-btn-xs data-count-edit" lay-event="edit">发货</a>
    {{#  } }}
    @endverbatim 
...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#dataTable',
            url: '/backend/product/onlineshop_order/getPageList',
            ...
            cols: [[
                ...
                {field: 'title',  title: '订单商品'},
                {field: 'total_fee',  title: '总金额'},
                {field: 'address',  title: '收货地址'},
                {field: 'express',  title: '快递信息'},
        ...
        table.on('toolbar(dataTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/onlineshop_order/addItem',
                    end: function(){
                        table.reload('dataTable')
                    }
                });
        ...
        table.on('tool(dataTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/onlineshop_order/viewItem/'+data.id,
                    end: function(){
                        table.reload('dataTable')
                    }
                });
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.post('/backend/product/onlineshop_order/delItem/'+data.id, {}, function(result){
