    ...
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: 'deduction_price',  title: '抵扣金额'},
                {field: 'thumb',  title: '缩略图',templet: function(data){
                    return '<img with="60px" src="'+data.thumb+'"/>'
                }},
                {field: 'thumb',  title: '使用商品',templet: function(data){
                    let  html = '<img width="60" src="'+data.goods_thumb+'" />'
                      html += '<p>'+data.goods_title+'</p>'
                    return html
                }},
                {field: 'operator_agentactive_vip',  title: '间接激活会员佣金'},
                
                {field: 'operator_commission',  title: '运营商激活会员佣金'},
                {field: 'agent_commission',  title: '代理商激活会员佣金'},
                {field: 'agent_price',  title: '代理商价格'},
                {field: 'operator_price',  title: '运营商价格'},
        ...
        form.on('switch(stateSwitch)', function(obj){
            ...
            $.post('/backend/product/couponshop_goods/setItem/' + obj.value ,datajson,function(data){
                layer.msg(data.msg);
                if(data.code !== 0) {
        ...
        table.on('tool(dataTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/couponshop_goods/viewItem/'+data.id,
                    end: function(){
                        table.reload('dataTable')
                    }
                });
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.post('/backend/product/couponshop_goods/delItem/'+data.id, {}, function(result){
