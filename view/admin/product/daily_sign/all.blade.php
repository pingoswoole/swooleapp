...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#dataTable',
            url: '/backend/product/daily_sign/getPageList',
            ...
            cols: [[
                ...
                {field: 'provider',  title: '支付方式'},
                {field: 'ordersn',  title: '平台订单号'},
                {field: 'out_trade_no',  title: '第三方订单号'},
                {field: 'total_fee',  title: '金额'},
                {field: 'order_id',  title: '支付订单表ID'},
                {field: 'out_trade_no',  title: '第三方订单号'},
        ...
        table.on('toolbar(dataTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/daily_sign/addItem',
                    end: function(){
                        table.reload('dataTable')
                    }
                });
        ...
        form.on('switch(stateSwitch)', function(obj){
            ...
            $.post('/backend/product/daily_sign/setItem/' + obj.value ,datajson,function(data){
                layer.msg(data.msg);
                if(data.code !== 0) {
        ...
        table.on('tool(dataTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/daily_sign/viewItem/'+data.id,
                    end: function(){
                        table.reload('dataTable')
                    }
                });
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.post('/backend/product/daily_sign/delItem/'+data.id, {}, function(result){
