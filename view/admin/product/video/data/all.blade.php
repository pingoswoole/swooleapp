...
<script type="text/html" id="resourceTpl">
    ...
    @verbatim
    {{#  layui.each(d.resource_urls, function(index, item){ }}
        <li>
            {{#  if(d.type === 'image'){ }}
...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#dataTable',
            url: '/backend/product/video_data/getPageList',
            ...
            cols: [[
                ...
                {field: 'cate_title',  title: '分类', templet: function(data){
                    return data.category? data.category.title : '';
                }},
                {field: 'type_name',  title: '资源类型'},
                {field: 'city_name',  title: '发布城市', templet: function(data){
                    return data.area? data.area.title : ''
                }},
                {field: 'title',  title: '名称'},
                {field: 'type_name',  title: '资源【多图或视频】',  templet: '#resourceTpl' },
                /* {field: 'thumb',  title: '封面',   templet: function(data){
        ...
        table.on('toolbar(dataTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/video_data/addItem',
                    end: function(){
                        table.reload('dataTable')
                    }
                });
        ...
        form.on('switch(stateSwitch)', function(obj){
            ...
            $.post('/backend/product/video_data/setItem/' + obj.value ,datajson,function(data){
                layer.msg(data.msg);
                if(data.code !== 0) {
        ...
        table.on('tool(dataTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/video_data/viewItem/'+data.id,
                    end: function(){
                        table.reload('dataTable')
                    }
                });
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.post('/backend/product/video_data/delItem/'+data.id, {}, function(result){
