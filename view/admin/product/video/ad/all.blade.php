...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#dataTable',
            url: '/backend/product/video_ad/getPageList',
            ...
            cols: [[
                ...
                {field: 'thumb',  title: '缩略图', templet: function(data){
                    return (data.thumb ?  '<img src="'+data.thumb+'" >' : '')
                }},
                {field: 'type_name',  title: '视频',  templet: '#resourceTpl' },
        ...
        form.on('switch(stateSwitch)', function(obj){
            ...
            $.post('/backend/product/video_ad/setItem/' + obj.value ,datajson,function(data){
                layer.msg(data.msg);
                if(data.code !== 0) {
        ...
        table.on('toolbar(dataTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/video_ad/addItem',
                    end: function(){
                        table.reload('dataTable')
                    }
                });
        ...
        table.on('tool(dataTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['90%', '90%'],
                    content: '/backend/product/video_ad/viewItem/'+data.id,
                    end: function(){
                        table.reload('dataTable')
                    }
                });
                ...
                layer.confirm('真的删除行么', function (index) {
                    $.post('/backend/product/video_ad/delItem/'+data.id, {}, function(result){
